...
One-time setup this repo still needs before that workflow can actually publish: **Settings → Pages → Source: “GitHub Actions”**. That can't be turned on yet, though — **GitHub Pages requires a public repo on the free plan, and this repo is currently private** (confirmed via the API: enabling Pages returns `422: Your current plan does not support GitHub Pages for this repository`). Making the repo public is the obvious fix once it's ready to publish (matches the spec's "v1: GitHub only, open-source" intent), but that's a one-way, visible decision left for you to make deliberately, not something this setup does on its own. Until then, the repo-root `manifest.xml` (pointing at `localhost`) is what you sideload locally per [Install / sideload](#install--sideload) above; a production manifest pointing at the Pages URL is generated at `dist/manifest.xml` by `npm run build`, ready for whenever the repo goes public and Pages can be enabled.
